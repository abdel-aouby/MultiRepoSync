...
for d in $(find "$CWD"  -maxdepth 2 -name ".git" -type d);
...
do
    ...
    if [ "$DEFAULT_BRANCH" == "$CURRENT_BRANCH" ]; then
        ...
        if [ "$save_master_answer" == "N" ] || [ "$save_master_answer" == "n" ] || [ -z ${save_master_answer+x} ]; then
            read -r -p "Do you want to update [${DEFAULT_BRANCH}] [Y/N]: " update_master
            read -r -p "Do you wish to save the same answer for further repositories with same situation? [Y/N]: " save_master_answer
        fi
        ...
        if [ "$save_no_master_answer" == "N" ] || [ "$save_no_master_answer" == "n" ] || [ -z ${save_no_master_answer+x} ]; then
            ...
            read -r -p "Enter the answer: " answer
            while [ "$answer" != 1 ] && [ "$answer" != 2 ] && [ "$answer" != 3 ] ; do
              echo -e "Wrong answer !! please try again."
              read -r -p "Enter the answer: " answer
            done

            read -r -p "Do you wish to save the same answer for further repositories with same situation? [Y/N]: " save_no_master_answer
        fi
